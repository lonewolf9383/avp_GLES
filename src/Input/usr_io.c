...
JOYSTICK_CONTROL_METHODS JoystickControlMethods =
{
	...
	0,//unsigned int JoystickAxis2IsMovement; // else it's looking
	0,//unsigned int JoystickAxis2IsTurning;	 // else it's sidestepping
	0,//unsigned int JoystickAxis2FlipVerticalAxis;
...
JOYSTICK_CONTROL_METHODS DefaultJoystickControlMethods =
{
	...
	0,//unsigned int JoystickAxis2VAxisIsMovement; // else it's looking
	0,//unsigned int JoystickAxis2POVHAxisIsTurning;	 // else it's sidestepping
	0,//unsigned int JoystickAxis2FlipVerticalAxis;

	DEFAULT_JOYSTICK_SENSITIVITY,//unsigned int Joystick axis 1 sensitivity;		  
	DEFAULT_JOYSTICK_SENSITIVITY,//unsigned int Joystick axis 2 sensitivity
...
//			playerStatusPtr->Mvt_InputRequests.Flags.Rqst_PauseGame = 1;
	...
	if (g_joystickEnabled == TRUE)
	{
		#define JOYSTICK_DEAD_ZONE 3000
		extern JOYINFOEX JoystickData;
		extern JOYCAPS JoystickCaps;
		
		if (JoystickData.dwYpos > JOYSTICK_DEAD_ZONE || JoystickData.dwYpos < -JOYSTICK_DEAD_ZONE)
		{
			if(JoystickControlMethods.JoystickVAxisIsMovement)
			{
				int speed = CalculateJoystickMoveSpeed(ControlMethods.MouseYSensitivity, -JoystickData.dwYpos);
				...
				playerStatusPtr->Mvt_MotionIncrement = speed;
			}
			else // looking up/down
			{
				int turnSpeed = CalculateJoystickTurnSpeed(ControlMethods.MouseYSensitivity, -JoystickData.dwYpos);
				if(JoystickControlMethods.JoystickFlipVerticalAxis) 
					turnSpeed = -turnSpeed;
				...
				playerStatusPtr->Mvt_AnaloguePitching = 1;
				playerStatusPtr->Mvt_PitchIncrement = turnSpeed;
			}
		}
		
		if (JoystickData.dwXpos > JOYSTICK_DEAD_ZONE || JoystickData.dwXpos < -JOYSTICK_DEAD_ZONE)
		{
			if (JoystickControlMethods.JoystickHAxisIsTurning)
			{
				int turnSpeed = CalculateJoystickTurnSpeed(ControlMethods.MouseXSensitivity, -JoystickData.dwXpos);
				...
				playerStatusPtr->Mvt_AnalogueTurning = 1;
				playerStatusPtr->Mvt_TurnIncrement = turnSpeed;
			}
			else // strafing
			{
				int speed = CalculateJoystickMoveSpeed(ControlMethods.MouseXSensitivity, JoystickData.dwXpos);
				if(speed < 0)
					playerStatusPtr->Mvt_InputRequests.Flags.Rqst_SideStepLeft = 1;
				else
					playerStatusPtr->Mvt_InputRequests.Flags.Rqst_SideStepRight = 1;
				
				playerStatusPtr->Mvt_SideStepIncrement = speed;
			}
		}
		
		if (JoystickData.dwYpos2 > JOYSTICK_DEAD_ZONE || JoystickData.dwYpos2 < -JOYSTICK_DEAD_ZONE)
		{
			if(JoystickControlMethods.JoystickAxis2IsMovement)
			{
				int speed = CalculateJoystickMoveSpeed(JoystickControlMethods.JoystickAxis2VerticalSensitivity, -JoystickData.dwYpos2);
				if(JoystickControlMethods.JoystickAxis2FlipVerticalAxis) 
					speed = -speed;
			...
			{
				// looking up/down
				int turnSpeed = CalculateJoystickTurnSpeed(JoystickControlMethods.JoystickAxis2VerticalSensitivity, -JoystickData.dwYpos2);
				if(JoystickControlMethods.JoystickAxis2FlipVerticalAxis) 
					turnSpeed = -turnSpeed;
		...
		if (JoystickData.dwXpos2 > JOYSTICK_DEAD_ZONE || JoystickData.dwXpos2 < -JOYSTICK_DEAD_ZONE)
		{
			if (JoystickControlMethods.JoystickAxis2IsTurning)
			{
				// Look left/right
				int turnSpeed = CalculateJoystickTurnSpeed(JoystickControlMethods.JoystickAxis2HorizontalSensitivity, -JoystickData.dwXpos2);
			...
			{
				int speed = CalculateJoystickMoveSpeed(JoystickControlMethods.JoystickAxis2HorizontalSensitivity, JoystickData.dwXpos2);
				if (speed < 0)
					playerStatusPtr->Mvt_InputRequests.Flags.Rqst_SideStepLeft = 1;
				else
					playerStatusPtr->Mvt_InputRequests.Flags.Rqst_SideStepRight = 1;
...
int CalculateJoystickTurnSpeed(int axisSensitivity, int axisSpeed)
{
	// This is not just any random number ive pulled out my arse. Its 1 / 33. 33 is the default sensitivity with
	// max sensitivity set at default*3. So sensitivity * sensitivityScaler makes a value between 0.0f & 3.0f;
	const float sensitivityScaler = 0.0303f;
	const float joyStickRange = 32768.0f;
	
	float velocity = (float)axisSpeed / joyStickRange;
	
	// squared velocity gives a nicer curve with less influence in lower values
	int finalVelocity =  (int)((velocity * velocity) * (((float)axisSensitivity * sensitivityScaler) * ONE_FIXED));
	
	// Turning axis need reversing so might as well do it here
	return ((axisSpeed < 0) ? finalVelocity : -finalVelocity);
}

inline int CalculateJoystickMoveSpeed(int axisSensitivity, int axisSpeed)
{
	// joystick range just so happens to be 1/2 the value of one unit of speed
	return axisSpeed * 2;	
}
