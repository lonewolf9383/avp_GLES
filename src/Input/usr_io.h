...
}CONTROL_METHODS;	// Mouse + affects joystick Axis 1 sensitivity
...
typedef struct
{
	...
	unsigned int JoystickAxis2IsMovement; // else it's looking
	unsigned int JoystickAxis2IsTurning;	 // else it's sidestepping
	unsigned int JoystickAxis2FlipVerticalAxis;

	unsigned int JoystickAxis2HorizontalSensitivity;		  
	unsigned int JoystickAxis2VerticalSensitivity; 
...
#define DEFAULT_JOYSTICK_SENSITIVITY 64
#define JOYSTICK_SENSITIVITY_MAX (DEFAULT_JOYSTICK_SENSITIVITY*3)
