...
typedef struct netgame_gamedata
{
	...
	NETGAME_PLAYERDATA playerData[MAX_PLAYERS];
	int teamScores[3];
...
{
	GAMEDESCRIPTION_PLAYERDATA players[MAX_PLAYERS];	
...
{
	int playerFrags[MAX_PLAYERS][MAX_PLAYERS];
	int playerScores[MAX_PLAYERS];
	int playerScoresAgainst[MAX_PLAYERS];
	int aliensKilled[MAX_PLAYERS][3];
	int deathsFromAI[MAX_PLAYERS];
...
// Sent from host to client to inform the client of its ID. It also contains the number of player details also included in
// the packed (following this struct)
PACKED_PUSH
typedef struct sclientidnet
{
	NetworkID clientsID;
	unsigned int numberOfPlayerDetailsFollowing;
} PACKED SClientIDNet;
PACKED_POP

PACKED_PUSH
typedef struct sclientidonlynet
{
	NetworkID clientID;
} PACKED SClientIDOnlyNet;
PACKED_POP

// Sent from client to host to inform host of the players name
PACKED_PUSH
typedef struct splayernamenet
{
	char playerName[MAX_PLAYER_NAME_LENGTH + 1];
} PACKED SPlayerNameNet;
PACKED_POP

// Sent from host to client on first connection informing client of existing players details
PACKED_PUSH
typedef struct splayerdetailsnet
{
	NetworkID clientID;
	char playerName[MAX_PLAYER_NAME_LENGTH + 1];
} PACKED SPlayerDetailsNet;
PACKED_POP
/* ---------------------------------------------------------------------
   Some prototypes
  ----------------------------------------------------------------------*/
...
extern int PlayerIdInPlayerList(NetworkID Id);
...
extern void RemovePlayerFromGame(NetworkID id);
...
extern void AddNetMsg_PlayerID(NetworkID playerID,unsigned char message);
extern void AddNetMsg_LastManStanding_RestartTimer(char time);
extern void AddNetMsg_LastManStanding_Restart(NetworkID alienID,int seed);
...
extern NetworkID myNetworkKillerId;
extern NetworkID myIgniterId;
extern NetworkID MultiplayerObservedPlayer;
