...
leaves the game... */
void RemovePlayersGhosts(NetworkID id)
{
...
STRATEGYBLOCK *FindGhost(NetworkID Id, int obId)
{
	extern int NumActiveStBlocks;
	extern STRATEGYBLOCK *ActiveStBlockList[];	
...
STRATEGYBLOCK *CreateNetGhost(NetworkID playerId, int objectId, VECTORCH *position, EULER* orientation, AVP_BEHAVIOUR_TYPE type, unsigned char IOType, unsigned char subtype)
{
	int i;
	STRATEGYBLOCK *sbPtr;
...
void ChangeGhostMarineAccoutrementSet(HMODELCONTROLLER *HModelController, NetworkID playerId)
{
	...
	/* data block */
	{
		...
		switch(AvP.PlayerType)
		{
			case(I_Marine):
			{
				...
				{
					ChangeGhostMarineAccoutrementSet(&corpseData->HModelController,g_netID);
				}
