...
static BOOL g_quitingGame = FALSE;
...
enum AVPMENU_ID g_menuToShowOnMainMenuRestart = AVPMENU_USERPROFILESELECT;
...
int AvP_MainMenus(void)
{
	...
	// If its the demo and the player completed a level before returning to the menu...
	...
	// If the player successfully completed a mission before re-entering the main menu...
	...
	SetupNewMenu(g_menuToShowOnMainMenuRestart);
   ...
   //	BezierCurve();
	...
	// Returning from a game back into the main menu or quiting entirely?
	if (AvPMenus.MenusState==MENUSSTATE_OUTSIDEMENUS)
	{
		// If we are quiting the game for good then roll the credits
		if (g_quitingGame)
		{
			// ATTN: Re-enable credits
			DoCredits();
		}
		else
		{
...
//      RenderMenuText("VS",330,95,16384,AVPMENUFORMAT_CENTREJUSTIFIED);
	...
	if (AvPMenus.CurrentMenu == AVPMENU_MULTIPLAYER_BEGINCONNECTION ||
		AvPMenus.CurrentMenu == AVPMENU_MULTIPLAYER_SPECIES_JOIN ||
...
void SetupNewMenu(enum AVPMENU_ID menuID)
{
	...
	switch (menuID)
	{
		...
		case AVPMENU_MULTIPLAYER_BEGINCONNECTION:
		{
			// Attempt to connect
			extern char IPAddressString[];
			extern char MP_PlayerName[];
			extern unsigned int g_netPortAddress;
			if (!NetConnect(IPAddressString, g_netPortAddress, MP_PlayerName))
			{
				// Connection failed.
				SetupNewMenu(AVPMENU_MULTIPLAYER_CONNECTFAILED);
				return;
			}
		...
		case AVPMENU_MULTIPLAYER_CONFIG :
		{
			//need to set the menu we return to
...
static void InteractWithMenuElement(enum AVPMENU_ELEMENT_INTERACTION_ID interactionID)
{
	...
	switch(elementPtr->ElementID)
	{
		...
		case AVPMENU_ELEMENT_QUIT_INGAME:
		{
			if (interactionID == AVPMENU_ELEMENT_INTERACTION_SELECT)
			{
		...
		case AVPMENU_ELEMENT_STARTMPGAME:
		{
			if (interactionID == AVPMENU_ELEMENT_INTERACTION_SELECT)
			{
				...
				if (NetHost(g_netPortAddress, 8, MP_PlayerName,MP_SessionName,MP_Species,netGameData.gameType,netGameData.levelNumber))
				{
					AvPMenus.MenusState = MENUSSTATE_STARTGAME;
				...
				{
					SetupNewMenu(AVPMENU_MULTIPLAYER_HOSTFAILED);
				}
			}
			break;
		}
...
//				netGameData.myGameState = NGS_Playing;
		...
		case AVPMENU_ELEMENT_VIDEOMODE:
		{
			if ((interactionID == AVPMENU_ELEMENT_INTERACTION_SELECT)
			  ||(interactionID == AVPMENU_ELEMENT_INTERACTION_INCREASE))
			{
				NextAvailableVideoMode(g_currentVideoX, g_currentVideoY, &g_currentVideoX, &g_currentVideoY);
			}
			else
			{
				PrevAvailableVideoMode(g_currentVideoX, g_currentVideoY, &g_currentVideoX, &g_currentVideoY);
			}
			break;
		}
...
static void RenderMenuElement(AVPMENU_ELEMENT *elementPtr, int e, int y)
{
	int (*RenderText)(char *textPtr, int sx, int sy, int alpha, enum AVPMENUFORMAT_ID format);
...
// Quit from ingame back into the main menu
void QuitInGameToMainMenu(enum AVPMENU_ID firstMenuToShow)
{
	AvP.MainLoopRunning = 0;
	g_quitingGame = FALSE; 		// We are leaving back to the main menu but not quiting the game completely
	g_menuToShowOnMainMenuRestart = firstMenuToShow;
}
