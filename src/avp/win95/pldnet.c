...
extern SPlayerName g_playerName;
...
NETGAME_GAMEDATA netGameData=
{
	...
	{{0},},	//NETGAME_PLAYERDATA playerData[MAX_PLAYERS];
	{0,},	//int teamScores[3];
...
NetworkID myNetworkKillerId = 0;
NetworkID myIgniterId = 0;
int MyHitBodyPartId=-1;
NetworkID MultiplayerObservedPlayer=0;
...
static void UpdateNetworkGameScores(NetworkID playerKilledId, NetworkID killerId,NETGAME_CHARACTERTYPE playerKilledType,NETGAME_CHARACTERTYPE killerType);
...
static void ProcessNetMsg_PlayerDescription(NETMESSAGE_PLAYERDESCRIPTION *msgPtr, NetworkID senderId);
static void ProcessNetMsg_StartGame(void);
static void ProcessNetMsg_PlayerState(NETMESSAGE_PLAYERSTATE *msgPtr, NetworkID senderId);
static void ProcessNetMsg_PlayerState_Minimal(NETMESSAGE_PLAYERSTATE_MINIMAL *msgPtr, NetworkID senderId,BOOL orientation);
static void ProcessNetMsg_FrameTimer(unsigned short frame_time, NetworkID senderId);
static void ProcessNetMsg_PlayerKilled(NETMESSAGE_PLAYERKILLED *msgPtr, NetworkID senderId);
static void ProcessNetMsg_PlayerDeathAnim(NETMESSAGE_CORPSEDEATHANIM* messagePtr,NetworkID senderId);
static void ProcessNetMsg_PlayerLeaving(NetworkID senderId);
...
static void ProcessNetMsg_LocalObjectState(NETMESSAGE_LOBSTATE *msgPtr, NetworkID senderId);
static void ProcessNetMsg_LocalObjectDamaged(char *msgPtr, NetworkID senderId);
static void ProcessNetMsg_LocalObjectDestroyed(NETMESSAGE_LOBDESTROYED *msgPtr, NetworkID senderId);
...
static void ProcessNetMsg_PlayerAutoGunState(NETMESSAGE_AGUNSTATE *msgPtr, NetworkID senderId);
static char *ProcessNetMsg_ChatBroadcast(char *subMessagePtr, NetworkID senderId);
...
static void ProcessNetMsg_PredatorSights(NETMESSAGE_PREDATORSIGHTS *messagePtr, NetworkID senderId);
...
static void ProcessNetMsg_LocalObjectOnFire(NETMESSAGE_LOBONFIRE *messagePtr, NetworkID senderId);
static void ProcessNetMsg_AlienAIState(NETMESSAGE_ALIENAISTATE *messagePtr, NetworkID senderId);
static void ProcessNetMsg_FarAlienPosition(NETMESSAGE_FARALIENPOSITION *messagePtr, NetworkID senderId);
static void ProcessNetMsg_SpotAlienSound(NETMESSAGE_SPOTALIENSOUND *messagePtr, NetworkID senderId);
static void ProcessNetMsg_LocalObjectDestroyed_Request(NETMESSAGE_LOBDESTROYED_REQUEST *messagePtr, NetworkID senderId);
...
static void ProcessNetMsg_Gibbing(NETMESSAGE_GIBBING* messagePtr,NetworkID senderId);
static void ProcessNetMsg_GhostHierarchyDamaged(char *messagePtr, NetworkID senderId);
static void ProcessNetMsg_MakeDecal(NETMESSAGE_MAKEDECAL *messagePtr);
static void ProcessNetMsg_AlienAISequenceChange(NETMESSAGE_ALIENSEQUENCECHANGE *messagePtr, NetworkID senderId);
static void ProcessNetMsg_AlienAIKilled(NETMESSAGE_ALIENAIKILLED *messagePtr, NetworkID senderId);
static void ProcessNetMsg_SpotOtherSound(NETMESSAGE_SPOTOTHERSOUND *messagePtr, NetworkID senderId);
void StartOfGame_PlayerPlacement(STRATEGYBLOCK *playerSbPtr,int seed);

static void CheckLastManStandingState();
static void Handle_LastManStanding_Restart(NetworkID alienID,int seed);
static void Handle_LastManStanding_RestartInfo(NetworkID alienID);
static void Handle_LastManStanding_LastMan(NetworkID marineID);
static void Handle_LastManStanding_RestartTimer(unsigned char time);

static void CheckSpeciesTagState();
static void Handle_SpeciesTag_NewPersonIt(NetworkID predatorID);
...
static void Inform_PlayerHasDied(NetworkID killer, NetworkID victim,NETGAME_CHARACTERTYPE killerType,char weaponIcon);
static void Inform_AiHasDied(NetworkID killer,ALIEN_TYPE type,char weaponIcon);
static void Inform_PlayerHasLeft(NetworkID player);
static void Inform_PlayerHasJoined(NetworkID player);
static void Inform_PlayerHasConnected(NetworkID player);
...
void InitAVPNetGame(void)
{
	...
	/* base initialisation of game description */	
	{
		...
		for(i=0;i<(MAX_PLAYERS);i++)
		{
			...
			for(j=0;j<(MAX_PLAYERS);j++) netGameData.playerData[i].playerFrags[j] = 0;
	...
	myNetworkKillerId = g_netID;	/* init global id of player who killed me last */
	...
	{
		netGameData.playerData[0].playerId = g_netID;
		strncpy(netGameData.playerData[0].name,g_playerName.playerName,NET_PLAYERNAMELENGTH);
...
//	ConvertNetNameToUpperCase(netGameData.playerData[0].name);
	...
	{
		int myIndex;
		myIndex = PlayerIdInPlayerList(g_netID);
...
{			
	NetworkID	dPlayFromId = 0;
	NetworkID 	dPlayToId = 0;
...
{			
	...
	if(netGameData.myGameState==NGS_Playing)
	{
		#if 0
		int myScore;
		int myIndex = PlayerIdInPlayerList(g_netID);
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
// ATTN: Delete ProcessSystemMessage
static void ProcessSystemMessage(char *msgP,unsigned int msgSize)
{
	...
	switch(systemMessage->dwType)
	{
		...
		case DPSYS_CREATEPLAYERORGROUP:
		{
			...
			{	
				...
				{
					NetworkID id = createMessage->netID;;
					char *name = createMessage->dpnName.playerName;
...
//					NewOnScreenMessage("A PLAYER HAS DISCONNECTED");
		...
		{			
			...
			{	
				...
				{
					NetworkID id = destroyMessage->netID;
...
void AddPlayerToGame(NetworkID id, char* name)
{
	int freePlayerIndex;
	...
	freePlayerIndex = PLAYER_INDEX(id);
...
void RemovePlayerFromGame(NetworkID id)
{
	int playerIndex,j;
	...
	char playerLeftMessage[MAX_PLAYER_NAME_LENGTH + 20];
	sprintf(playerLeftMessage, "%s has left the game", netGameData.playerData[playerIndex].name);
	NewOnScreenMessage(playerLeftMessage);
...
/*----------------------------------------------------------------------
	...
	Automatically called by the NetUpdate function when a game message is recieved
  ----------------------------------------------------------------------*/
void ProcessGameMessage(NetworkID senderId, char *msgP,unsigned int msgSize)
{
	char* subMessagePtr;
...
void NetSendMessages(void)
{
	...
	
	{
		...
		if(!netGameData.skirmishMode)
		{
			if(g_netID)
			{
				if (!NetSendData(NO_ID, TRUE, &sendBuffer, numBytes))
				{
...
void EndAVPNetGame(void)
{
	...
	{
		// TODO: Update
		//hres = IDirectPlay4_DestroyPlayer(glpDP, g_netID);
		g_netID = NULL;
	}
...
void AddNetMsg_PlayerState(STRATEGYBLOCK *sbPtr)
{
	...
	//check our carrent character type
	{
		...
		playerIndex = PlayerIdInPlayerList(g_netID);
...
void AddNetMsg_PlayerKilled(int objectId,DAMAGE_PROFILE* damage)
{
	...
	has killed himself, or the NetworkID of the killer (which may in fact be the player's NetworkID)*/
	...
	if(myNetworkKillerId)
	{
		...
		else
		{
			...
			myNetworkKillerId=g_netID;	
			messagePtr->killerId=0;
		}
	}
	
	if(!myNetworkKillerId || myNetworkKillerId==g_netID)
	{
	...
	Inform_PlayerHasDied(myNetworkKillerId,g_netID,messagePtr->killerType,messagePtr->weaponIcon);
...
void AddNetMsg_ChatBroadcast(char *string,BOOL same_species_only)
{
	...
	{
		sprintf(OnScreenMessageBuffer,"%s: %s",netGameData.playerData[PlayerIdInPlayerList(g_netID)].name,string);
...
void AddNetMsg_PlayerID(NetworkID playerID,unsigned char message) 
{
	NETMESSAGEHEADER *headerPtr;
...
void AddNetMsg_LastManStanding_Restart(NetworkID alienID,int seed) 
{
	NETMESSAGEHEADER *headerPtr;
...
{	
	...
	{ 
		...
		for(i=0;i<MAX_PLAYERS;i++)
		{
			...
			if(netGameData.playerData[i].playerId != messagePtr->players[i].playerId)
			{
				if(messagePtr->players[i].playerId)
				{
					DWORD size=0;
					char* data;
					HRESULT hr = DPERR_BUFFERTOOSMALL;
					...
					if(hr==DP_OK || hr==DPERR_BUFFERTOOSMALL)
					{
						...
						*data=0;
						...
						if(hr==DP_OK)
						{
							strncpy(netGameData.playerData[i].name,((SPlayerName*)data)->playerName,NET_PLAYERNAMELENGTH-1);	
		...
		if(netGameData.needGameDescription)
		{
			...
			int the player list*/
			if(PlayerIdInPlayerList(g_netID)!=NET_IDNOTINPLAYERLIST)
			{			
...
static void ProcessNetMsg_PlayerDescription(NETMESSAGE_PLAYERDESCRIPTION *messagePtr, NetworkID senderId)
{	
...
static void ProcessNetMsg_PlayerState(NETMESSAGE_PLAYERSTATE *messagePtr, NetworkID senderId)
{
	VECTORCH position;
	int playerIndex;
	STRATEGYBLOCK *sbPtr;
...
static void ProcessNetMsg_PlayerState_Minimal(NETMESSAGE_PLAYERSTATE_MINIMAL *messagePtr, NetworkID senderId,BOOL orientation)
{
	int playerIndex;
	STRATEGYBLOCK *sbPtr;
...
static void ProcessNetMsg_PlayerKilled(NETMESSAGE_PLAYERKILLED *messagePtr, NetworkID senderId)
{
	STRATEGYBLOCK *sbPtr;
	int senderPlayerIndex;
...
static void ProcessNetMsg_ScoreChange(NETMESSAGE_SCORECHANGE *messagePtr)
{
	...
	if(messagePtr->killerIndex == MAX_PLAYERS)
	{
		//killed by ai
...
static void ProcessNetMsg_LocalObjectState(NETMESSAGE_LOBSTATE *messagePtr, NetworkID senderId)
{
	STRATEGYBLOCK *sbPtr;
	int objectId;
...
static void ProcessNetMsg_LocalObjectDamaged(char *messagePtr, NetworkID senderId)
{
	NETMESSAGE_LOBDAMAGED_HEADER *messageHeader=0;
	NETMESSAGE_DAMAGE_PROFILE *messageProfile=0;
	NETMESSAGE_DAMAGE_MULTIPLE *messageMultiple=0;
	...
	if(messageHeader->playerId != g_netID)
	{
	...
	/* next we have to find this object in our strategyblock list */
	{
		...
		LOCALASSERT(myNetworkKillerId==NULL || myNetworkKillerId ==g_netID);
		/*
...
static void ProcessNetMsg_LocalObjectDestroyed_Request(NETMESSAGE_LOBDESTROYED_REQUEST *messagePtr, NetworkID senderId)
{
	...
	/* This message is for the player who owns the object, so first check 
	if the message is meant for us */
	if(messagePtr->playerId != g_netID) return;
...
static void ProcessNetMsg_LocalObjectDestroyed(NETMESSAGE_LOBDESTROYED *messagePtr, NetworkID senderId)
{
	STRATEGYBLOCK *sbPtr;
	int objectId;
...
static void ProcessNetMsg_EndGame(void)
{
	...
	/* only do this if we're playing or in startup */
			...
			{
				if(netGameData.playerData[PlayerIdInPlayerList(g_netID)].startFlag)
				{
					netGameData.myGameState = NGS_Playing;
				}
			}
...
static void ProcessNetMsg_PlayerAutoGunState(NETMESSAGE_AGUNSTATE *messagePtr, NetworkID senderId)
{
	STRATEGYBLOCK *sbPtr;
	int objectId;
...
/* returns index if the given NetworkID is in the player list */
int PlayerIdInPlayerList(NetworkID Id)
{
	int i;
...
static void UpdateNetworkGameScores(NetworkID playerKilledId, NetworkID killerId,NETGAME_CHARACTERTYPE playerKilledType,NETGAME_CHARACTERTYPE killerType)
{
	...
	{
		//killed by alien ai
		AddNetMsg_ScoreChange(MAX_PLAYERS,playerKilledIndex);
	}
	else
	{
...
void DoNetScoresForHostDeath(NETGAME_CHARACTERTYPE myType,NETGAME_CHARACTERTYPE killerType)
{
	...
	UpdateNetworkGameScores(g_netID,myNetworkKillerId,myType,killerType);
}
...
int AddUpPlayerFrags(int playerId)
{
	...
	for(j=0;j<(MAX_PLAYERS);j++) score+=netGameData.playerData[playerId].playerFrags[j];
	return score;
}
...
void TeleportNetPlayerToAStartingPosition(STRATEGYBLOCK *playerSbPtr, int startOfGame)
{
	...
	if(startOfGame)
	{
		numReadThro = (PlayerIdInPlayerList(g_netID))%8;
		while(numReadThro<0)
		{
	...
	for(start_index=FastRandom()%numStartPositions;numChecked<numStartPositions;start_index++,numChecked++)
	{
		...
		for(playerIndex=0;playerIndex<MAX_PLAYERS;playerIndex++)
		{
			if(netGameData.playerData[playerIndex].playerId &&
			   netGameData.playerData[playerIndex].playerId!=g_netID &&	
...
void StartOfGame_PlayerPlacement(STRATEGYBLOCK *playerSbPtr,int seed)
{
	...
	VECTORCH ChosenPositions[MAX_PLAYERS];
	int NumChosen=0;
	int i;
	...
	for(i=0;i<MAX_PLAYERS;i++)
	{
		...
		if(netGameData.playerData[i].playerId==g_netID)
		{
			extern MODULE * playerPherModule;
...
int DetermineAvailableCharacterTypes(BOOL ConsiderUsedCharacters)
{
	...
	if(ConsiderUsedCharacters)
	{
		if(AvP.Network!=I_No_Network)
		{
			...
			for(i=0;i<MAX_PLAYERS;i++)
			{
				if(netGameData.playerData[i].playerId && netGameData.playerData[i].playerId!=g_netID)
				{
					switch(netGameData.playerData[i].characterType)	
					{
						case NGCT_Marine :
...
void SpeciesTag_DetermineMyNextCharacterType()
{
	...
	if(myNetworkKillerId && myNetworkKillerId!=g_netID && CountPlayersOfType(tagSpecies)==1)
	{
...
static int CountMultiplayerLivesLeft()
{
	...
	{
		int index=PlayerIdInPlayerList(g_netID);
...
void GetNextMultiplayerObservedPlayer()
{
	...
	else
	{
		...
		for(next_index=cur_index+1;count<MAX_PLAYERS;next_index++,count++)
		{
			if(next_index==MAX_PLAYERS) next_index=0;
...
static void PeriodicScoreUpdate()
{
	...
	if(netGameData.myGameState!=NGS_Playing) return;
	for(playerIndex++;count<MAX_PLAYERS;count++,playerIndex++)
	{
		if(playerIndex>=MAX_PLAYERS && netGameData.gameType==NGT_CoopDeathmatch)
		{
