...
static void FlushTriangleBuffers(int backup)
{
	...
	if (tarrc) 
	{
		glDrawElements(GL_TRIANGLES, tarrc*3, GL_UNSIGNED_SHORT, tarr);
		tarrc = 0;
		tarrp = tarr;
		varrc = 0;
		varrp = varr;
	...
	if (starrc) {
		...
		glDrawElements(GL_TRIANGLES, starrc*3, GL_UNSIGNED_SHORT, starr);
...
{
	return;
	const float x[4] = {-0.8f, 0.8f, 0.8f -0.8f};
	const float y[4] = {0.2f, 0.2f, 0.8f, 0.8f};
	CheckTriangleBuffer(4, 2, 0, 0, NULL, TRANSLUCENCY_OFF, FILTERING_BILINEAR_ON);
	...
	/*varrp->v[0] = (Global_VDB_Ptr->VDB_ClipLeft * dest->left) / 255.0f;
	varrp->v[0] = (varrp->v[0] - ScreenDescriptorBlock.SDB_CentreX) / ScreenDescriptorBlock.SDB_CentreX;
	varrp->v[1] = -(((Global_VDB_Ptr->VDB_ClipUp * dest->top) / 255.0f) - ScreenDescriptorBlock.SDB_CentreY) / ScreenDescriptorBlock.SDB_CentreY;
	varrp->v[2] = -1.0f;
	varrp->v[3] = 1.0f;
	++varrp;
	++varrc;
	
	varrp->v[0] = (Global_VDB_Ptr->VDB_ClipRight * dest->right) / 255.0f;
	varrp->v[0] = (varrp->v[0] - ScreenDescriptorBlock.SDB_CentreX) / ScreenDescriptorBlock.SDB_CentreX;
	varrp->v[1] = -(((Global_VDB_Ptr->VDB_ClipUp * dest->top) / 255.0f) - ScreenDescriptorBlock.SDB_CentreY) / ScreenDescriptorBlock.SDB_CentreY;
	varrp->v[2] = -1.0f;
	varrp->v[3] = 1.0f;
	...
	varrp->v[0] = (Global_VDB_Ptr->VDB_ClipRight * dest->right) / 255.0f;
	varrp->v[0] = (varrp->v[0] - ScreenDescriptorBlock.SDB_CentreX) / ScreenDescriptorBlock.SDB_CentreX;
	varrp->v[1] = -(((Global_VDB_Ptr->VDB_ClipDown * dest->bottom) / 255.0f) - ScreenDescriptorBlock.SDB_CentreY) / ScreenDescriptorBlock.SDB_CentreY;
	varrp->v[2] = -1.0f;
	varrp->v[3] = 1.0f;
	++varrp;
	++varrc;
