...
static unsigned short svarrc, starrc;
...
void FlushTriangleBuffers(int backup)
{
	int i;
...
void ThisFramesRenderingHasFinished()
{
	...
	if (Global_VDB_Ptr)
		DrawDebugLine(&Global_VDB_Ptr->VDB_World, &GunMuzzleDirectionInWS, 10000);
}
...
void D3D_Rectangle(int x0, int y0, int x1, int y1, int r, int g, int b, int a)
{
	...
	CheckTriangleBuffer(4, 2, 0, 0, NULL, TRANSLUCENCY_GLOWING, -1);

	float top = -((float)y0 - ScreenDescriptorBlock.SDB_CentreY) / ScreenDescriptorBlock.SDB_CentreY;
	float bottom = -(((float)y1 - 1.0f) - ScreenDescriptorBlock.SDB_CentreY) / ScreenDescriptorBlock.SDB_CentreY;
	float left = ((float)x0 - ScreenDescriptorBlock.SDB_CentreX) / ScreenDescriptorBlock.SDB_CentreX;
	float right = (((float)x1 - 1.0f) - ScreenDescriptorBlock.SDB_CentreX) / ScreenDescriptorBlock.SDB_CentreX;
...
void D3D_PlayerOnFireOverlay()
{
	...
	const float uOffset[4] = {0.0f, 1.0f, 1.0f, 0.0f};
	const float vOffset[4] = {0.0f, 0.0f, 1.0f, 1.0f};
	float u = (FastRandom()&255)/256.0f;
	float v = (FastRandom()&255)/256.0f;
	
	D3DTexture *pTex = ImageHeaderArray[BurningImageNumber].D3DTexture;
	CheckTriangleBuffer(4, 2, 0, 0, pTex, TRANSLUCENCY_GLOWING, FILTERING_BILINEAR_ON);
	
	int i;
	for (i = 0; i < 4; ++i)
	{
		varrp->c[0] = (colour >> 0)  & 0xFF;
		varrp->c[2] = (colour >> 8)  & 0xFF;
		varrp->c[3] = (colour >> 16)  & 0xFF;
		varrp->c[4] = (colour >> 24)  & 0xFF;
		...
		varrp->t[0] = u + uOffset[i];
		varrp->t[1] = v + vOffset[i];
		++varrp;
		++varrc;
	}
...
{
	const int theta[2] = {(CloakingPhase/8)&4095, (800-CloakingPhase/8)&4095};
	const float x[4] = {-1.0f, 1.0f, 1.0f, -1.0f};
	const float y[4] = {-1.0f, -1.0f, 1.0f, 1.0f};
	...
	D3DTexture *pTex = ImageHeaderArray[SpecialFXImageNumber].D3DTexture;
	CheckTriangleBuffer(4, 2, 0, 0, pTex, TRANSLUCENCY_INVCOLOUR, FILTERING_BILINEAR_ON);
	...
	for (i = 0; i < 2; i++) 
	{
		...
		for (vert = 0; vert < 4; ++vert)
		{
			...
			varrp->t[0] = 0.875f + (cos*(x[vert]) - sin*(y[vert]));
			varrp->t[1] = 0.375f + (sin*(x[vert]) + cos*(y[vert]));
			
			varrp->c[2] = (colour >> 0)  & 0xFF;
			varrp->c[1] = (colour >> 8)  & 0xFF;
			varrp->c[0] = (colour >> 16)  & 0xFF;
			varrp->c[3] = (colour >> 24)  & 0xFF;
			++varrp;
			++varrc;
		}
...
void DrawNoiseOverlay(int tr)
{
	...
	const float y[4] = {-1.0f, -1.0f, 1.0f, 1.0f};
	const int size = 256;
	float uBase = FastRandom()&255;
	float vBase = FastRandom()&255;
	const float u[4] = {uBase / 256.0f, (uBase + size) / 256.0f, (uBase + size) / 256.0f, uBase / 256.0f};
	const float v[4] = {vBase / 256.0f, vBase / 256.0f, (vBase + size) / 256.0f, (vBase + size) / 256.0f};
		
	D3DTexture *pTex = ImageHeaderArray[StaticImageNumber].D3DTexture;
	CheckTriangleBuffer(4, 2, 0, 0, pTex, TRANSLUCENCY_GLOWING, FILTERING_BILINEAR_ON);
	...
	for (i = 0; i < 4; ++i)
	{
		varrp->c[0] = varrp->c[1] = varrp->c[2] = 255;
		varrp->c[3] = tr;
		varrp->v[0] = x[i];
		varrp->v[1] = y[i];
		varrp->v[2] = 1.0f;
...
void D3D_ScreenInversionOverlay()
{
	...
	D3DTexture *pTex = ImageHeaderArray[SpecialFXImageNumber].D3DTexture;
	CheckTriangleBuffer(8, 4, 0, 0, pTex, TRANSLUCENCY_DARKENINGCOLOUR, FILTERING_BILINEAR_ON);
	...
	for (i = 0; i < 2; i++) 
	{
		...
		for (vert = 0; vert < 4; ++vert)
		{
			...
			varrp->v[2] = -1.0f;
			varrp->v[3] = 1.0f;
			varrp->t[0] = 0.375f + (cos * (x[vert]) - sin * (y[vert]));
			varrp->t[1] = 0.375f + (sin * (x[vert]) + cos * (y[vert]));
			varrp->c[0] = varrp->c[1] = varrp->c[2] = varrp->c[3] = 255;
			++varrp;
			++varrc;
		}
...
{
	const float size = 128.0f*(1.0f-level*0.8f);
	int alpha = 64.0f + level * 64.0f;
	
	const float x[4] = {-1.0f, 1.0f, 1.0f, -1.0f};
	const float y[4] = {-1.0f, -1.0f, 1.0f, 1.0f};
	const float u[4] = {(128.0f - size) / 256.0f, (128.0f - size) / 256.0f, (128.0f + size) / 256.0f, (128.0f + size) / 256.0f};
	const float v[4] = {1.0f, 1.0f, 1.0f, 1.0f};
	
	D3DTexture *pTex = ImageHeaderArray[PredatorNumbersImageNumber].D3DTexture;
	
	FlushTriangleBuffers(0);
	CheckTranslucencyModeIsCorrect(TRANSLUCENCY_NORMAL);
	CheckBoundTextureIsCorrect(pTex);
	CheckFilteringModeIsCorrect(FILTERING_BILINEAR_ON);
	pglDepthFunc(GL_ALWAYS);
...
void D3D_FadeDownScreen(int brightness, int colour)
{
	...
	CheckTriangleBuffer(4, 2, 0, 0, NULL, TRANSLUCENCY_NORMAL, FILTERING_BILINEAR_ON);
...
/* possibly use polygon offset? (predator hud) */
	...
	CheckTriangleBuffer(4, 2, 0, 0, tex, TRANSLUCENCY_GLOWING, tex->filter);
	...
	for (i = 0; i < 4; i++) 
	{
		...
		varrp->v[0] =  ((float)quadVerticesPtr[i].X - ScreenDescriptorBlock.SDB_CentreX)/ScreenDescriptorBlock.SDB_CentreX;
		varrp->v[1] = -((float)quadVerticesPtr[i].Y - ScreenDescriptorBlock.SDB_CentreY)/ScreenDescriptorBlock.SDB_CentreY;
		varrp->v[2] = -1.0f;
		varrp->v[3] = 1.0f;
		
		varrp->t[0] = ((float)quadVerticesPtr[i].U)*RecipW;
		varrp->t[1] = ((float)quadVerticesPtr[i].V)*RecipH;
		++varrp;
		++varrc;
	}
	tarrp->a = varrc - 4;
...
int Hardware_RenderSmallMenuText(char *textPtr, int sx, int sy, int alpha, enum AVPMENUFORMAT_ID format) 
{
	switch(format)
	{
		default:
...
//	LOCALASSERT(x>0);

	{
		...
		D3D_RenderHUDString(textPtr,sx,sy,colour);
	}
	return sx;
}
...
void D3D_DrawColourBar(int yTop, int yBottom, int rScale, int gScale, int bScale)
{
	...
	CheckTriangleBuffer(4*255, 2*255, 0, 0, NULL, TRANSLUCENCY_OFF, FILTERING_BILINEAR_ON);
	...
	for (i = 0; i < 255; ) 
	{
		...
		varrp->c[0] = MUL_FIXED(c, rScale);
		varrp->c[1] = MUL_FIXED(c, gScale);
		varrp->c[2] = MUL_FIXED(c, bScale);
		varrp->c[3] = 255;
		
		varrp->v[0] = (Global_VDB_Ptr->VDB_ClipRight*i) / 255.0f;
		varrp->v[0] =  (varrp->v[0] - ScreenDescriptorBlock.SDB_CentreX)/ScreenDescriptorBlock.SDB_CentreX;
		varrp->v[1] = -((float)yTop - ScreenDescriptorBlock.SDB_CentreY)/ScreenDescriptorBlock.SDB_CentreY;
		varrp->v[2] = -1.0f;
		varrp->v[3] = 1.0f;
		++varrp;
		++varrc;
		...
		varrp->v[0] = (Global_VDB_Ptr->VDB_ClipRight*i) / 255.0f;
		varrp->v[0] =  (varrp->v[0] - ScreenDescriptorBlock.SDB_CentreX)/ScreenDescriptorBlock.SDB_CentreX;
		varrp->v[1] = -((float)yBottom - ScreenDescriptorBlock.SDB_CentreY)/ScreenDescriptorBlock.SDB_CentreY;
		varrp->v[2] = -1.0f;
		varrp->v[3] = 1.0f;
		++varrp;
		++varrc;
...
{
	const float x[4] = {x0, x1 - 1, x1 - 1, x0};
	const float y[4] = {y0, y0, y1 - 1, y1 - 1};
		
	CheckTriangleBuffer(4, 2, 0, 0, NULL, TRANSLUCENCY_OFF, FILTERING_BILINEAR_ON);

	int i;
	for (i = 0; i < 4; ++i)
	{
		varrp->c[0] = ((FillColour >> 0)  & 0xFF);
		varrp->c[1] = ((FillColour >> 8)  & 0xFF);
		varrp->c[2] = ((FillColour >> 16)  & 0xFF);
		varrp->c[3] = 255;
		
		varrp->v[0] = (x[i] - ScreenDescriptorBlock.SDB_CentreX) / ScreenDescriptorBlock.SDB_CentreX;
		varrp->v[1] = -(y[i] - ScreenDescriptorBlock.SDB_CentreY) / ScreenDescriptorBlock.SDB_CentreY;
		varrp->v[2] = -1.0f;
		varrp->v[3] = 1.0f;
...
// Render an image using two triangles stretching the texture between them. dest* should be between -1.0f/+1.0f
void RenderScaledImage(float destLeft, float destRight, float destTop, float destBottom, D3DTexture *pTex, const RECT *src)
{
	FlushTriangleBuffers(0);
	CheckTranslucencyModeIsCorrect(TRANSLUCENCY_NORMAL);
	CheckBoundTextureIsCorrect(pTex);
	CheckFilteringModeIsCorrect(pTex->filter);
	
	float uvLeft = (float)src->left / pTex->w;
	float uvRight = (float)src->right / pTex->w;
	float uvTop = (float)src->top / pTex->h;
	float uvBottom = (float)src->bottom / pTex->h;
...
void DrawDebugLine(const VECTORCH *pFrom, const VECTORCH *pDir, int lineLength)
{
	return;
 	CheckBoundTextureIsCorrect(NULL);
	float positions[2][4];
	
	GLfloat ZNear = (float) (Global_VDB_Ptr->VDB_ClipZ * GlobalScale);
	GLfloat x, y, z;
	GLfloat w;
	
	x = pFrom->vx - Global_VDB_Ptr->VDB_World.vx;
	y = pFrom->vy - Global_VDB_Ptr->VDB_World.vy;
	z = pFrom->vz - Global_VDB_Ptr->VDB_World.vz;
	
	// Project line from start point
	VECTORCH to;
 	to.vx = pFrom->vz + (MUL_FIXED(pDir->vx, lineLength));
	to.vy = pFrom->vy + (MUL_FIXED(pDir->vy, lineLength));
	to.vz = pFrom->vz + (MUL_FIXED(pDir->vz, lineLength));
	
	if (z != 0.0f)
		x =  ((float)x*((float)Global_VDB_Ptr->VDB_ProjX+1.0f))/((float)z*(float)ScreenDescriptorBlock.SDB_CentreX);
	else
		x = 0.0f;
		
	if (z != 0.0f)
		y = -((float)y*((float)Global_VDB_Ptr->VDB_ProjY+1.0f))/((float)z*(float)ScreenDescriptorBlock.SDB_CentreY);
	else
		y = 0.0f;
	...
	x =  ((float)to.vx*((float)Global_VDB_Ptr->VDB_ProjX+1.0f))/((float)to.vz*(float)ScreenDescriptorBlock.SDB_CentreX);
	y = -((float)to.vy*((float)Global_VDB_Ptr->VDB_ProjY+1.0f))/((float)to.vz*(float)ScreenDescriptorBlock.SDB_CentreY);
	z = 1.0f - 2.0f * ZNear/to.vz;
	w = (float)to.vz;
	positions[1][0] = x*w;
	positions[1][1] = y*w;
	positions[1][2] = z*w;
	...
	static const unsigned short pIndex[] = {1, 1};
	//pglDepthFunc(GL_ALWAYS);
	pglVertexPointer(4, GL_FLOAT,0, positions);
	pglDisableClientState(GL_TEXTURE_COORD_ARRAY);
	pglDisableClientState(GL_COLOR_ARRAY);
	
	pglDrawElements(GL_POINTS, 1, GL_UNSIGNED_SHORT, pIndex);
